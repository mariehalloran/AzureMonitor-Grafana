...
import {
  getDataSourcesVariableForType,
  getResourceGraphVariable,
  getSubscriptionVariable,
} from '../Variables/variables';

export function getGenericSceneAppPage(
  title: string,
  url: string,
      ...
      children: [
        new SceneFlexItem({
          ...
          body: PanelBuilders.text()
            ...
            .setOption(
              'content',
              `> ### ⚠️ No ${missingDs} datasources found <br>\n> ##### This page requires ${requiredDatasources} to work.  \n> ##### Please go to [connections](/connections/datasources) configure the appropriate datasources. For more information, see our [documentation](/plugins/${AZURE_MONITORING_PLUGIN_ID})`
            )
...
export function getSharedSceneVariables(drilldownScene: boolean) {
  ...
  variables.push(
    getResourceGraphVariable(
      azure_monitor_queries['clustersVariableQuery'],
      `\$${SUBSCRIPTION_VARIABLE}`,
      CLUSTER_VARIABLE,
      'AKS Cluster',
      'Select a cluster',
...
export function getBehaviorsForVariables(
  variables: Array<DataSourceVariable | QueryVariable | CustomVariable | TextBoxVariable>,
  pluginReporter: Reporter
) {
 ...
 */
export function variableShouldBeCleared(
  variableOptions: VariableValueOption[],
  currentVariableValue: VariableValue,
  variableIsLoading: boolean | undefined
) {
